...
class BST(BinaryTree):
    ...
    def __repr__(self):
        ...
        to recreate a valid instance of the class.
        Thus, if you create
        a variable using the command BST([1,2,3])
    ...
    def __next__(self):
        if self.index < self.to_list('inorder').__len__():
            tbr = self.to_list('inorder')[self.index]
            self.index += 1
            return tbr
        else:
            raise StopIteration
    ...
    def _is_bst_satisfied(node):
        ...
        because those are static methods just like this one.
        '''
        ret = True
        if node.left:
    ...
    def _remove(node, value):
        ...
        if value > node.value:
            ...
            if node.right is None and node.left is None:
                ...
                node.value = succ
                node.right = BST._remove(node.right, succ)
                return
